...
async function sendMessage() {
    ...
    if (userMessage.length) {
        ...
        // Get current API key (obfuscated)
        const currentApiKey = window.CONFIG?.getKey() || "";
...
window.addEventListener('load', () => {
    ...
    console.log('API Key status:', window.CONFIG?.getKey ? '✅ Available (obfuscated)' : '❌ Missing');
...
cat > config.js << EOF
// Generated by GitHub Actions - obfuscated for security
window.CONFIG = {
  getKey: function() {
    const parts = ['${GEMINI_API_KEY:0:10}', '${GEMINI_API_KEY:10:10}', '${GEMINI_API_KEY:20:10}', '${GEMINI_API_KEY:30}'];
    return parts.join('');
  },
...
// Prevent direct access to config.js
