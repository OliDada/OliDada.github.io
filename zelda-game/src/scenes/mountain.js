...
export default async function mountain(k) {
    ...
    const mapData = await fetchMapData('./assets/maps/mountain.json');
    ...
    for (const layer of layers) {
        ...
        if (layer.name === 'SpawnPoints') {
            for (const object of layer.objects) {
                ...
                if (
                    object.name === 'player-village' &&
                    !entities.player &&
                    previousScene !== 'forest'
                ) {
                ...
                if (
                    object.name === 'player-forest' &&
                    !entities.player &&
                    previousScene === 'forest' &&
                    previousScene !== 'village'
                ) {
                    entities.player = map.add(
                        generatePlayerComponents(k, k.vec2(object.x, object.y))
                    );
    ...
    entities.player.onCollide('forest-entrance', () => {
        gameState.setPreviousScene('mountain');
        k.go('forest');
    });
    ...
    for (const slime of entities.slimes) {
        ...
        // Add a pullable object near the player if none exists
        if (entities.pullables.length === 0 && entities.player) {
                const pullable = [
                    k.sprite('sprites', { frame: 0 }), // Use a generic sprite, adjust as needed
                    k.area({ shape: new k.Rect(k.vec2(0, 4), 16, 16) }),
                    k.pos(entities.player.pos.x - 32, entities.player.pos.y),
                    k.body(),
                    { areaWidth: 16, areaHeight: 16 },
                    'pullable',
                    'solid',
                ];
                const pullableObj = k.add(pullable); // Register globally
                entities.pullables.push(pullableObj);
        }
