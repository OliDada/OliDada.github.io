...
export default async function trollDinner(k) {
    ...
    if (!currentSong || !currentSong.name || currentSong.name !== "troll-dinner-soundtrack") {
        gameState.pauseCurrentSong();
        const newSong = k.play("troll-dinner-soundtrack", { loop: true });
        newSong.name = "troll-dinner-soundtrack"; // Attach a name property for tracking
