...
export default async function beanStalk(k) {

    beanStalkState.setBeanStalkHeight(5); 
    gameState.setPlantedBeans(true);
    ...
    const mapData = await fetchMapData("./assets/maps/bean-stalk.json");
    ...
    for (const layer of layers) {
        ...
        if (layer.name === "SpawnPoints") {
            for (const object of layer.objects) {
                ...
                if (object.name === "player" && !entities.player && previousScene !== "clouds") {
                    entities.player = map.add(generatePlayerComponents(k, k.vec2(object.x, object.y)));
                    continue;
                }
                if (object.name === "player-clouds" && !entities.player && previousScene === "clouds") {
                    entities.player = map.add(generatePlayerComponents(k, k.vec2(object.x, object.y)));
                }
            }
            continue;
    ...
    entities.player.onCollide("clouds-entrance", () => {
