...
export default async function tavern(k) {
    ...
    for (const layer of layers) {
        ...
        if (layer.name === "SpawnPoints") {
            for (const object of layer.objects) {
                ...
                //if (object.name === "player-tavern" && !entities.player && previousScene === "barn-tavern-second-floor") {
                //    entities.player = map.add(generatePlayerComponents(k, k.vec2(object.x, object.y)));
                //    continue;
                //}
            }
    ...
    entities.player.onCollide("bartender", () => {
        startInteraction(k, entities.bartender, entities.player);
    ...
    entities.player.onCollide("tavern-second-floor", () => {
    ...
    console.log("Tilesets in tavern map:", mapData.tilesets);
}